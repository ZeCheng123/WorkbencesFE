<template>
  <div ref="mainRef" class="main">
    <span class="header">
      ...
      <span class="step">
        <span class="item">
          ...
          <span :class="currentStep >= 2 ? 'name_selected' : 'name'">待处理</span>
        </span>
        <!-- <span class="item">
        ...
        <span class="item">
          <span @click="currentStepToPeiSong" :class="currentStep == 4 ? 'num_selected' : 'num'">3</span>
    ...
    <span class="task_info">
      <span class="title">任务详情
        <el-button v-if="currentStep >= 5" type="primary" class="primary_blue_btn">已派工配送</el-button><el-button
          v-if="currentStep == 6" type="primary" class="primary_green_btn">已派工安装</el-button></span>
      ...
      <span class="table_content">
        <el-table :data="tableDataDispatch" :stripe="false" style="width: 100%" max-height="115">
          ...
          <el-table-column prop="stage__c" label="状态">
            <template #default="scope">
              <span style="color: blue;">{{ scope.row["stage__c"] == "0" ? "待开始" : scope.row["stage__c"] == "1" ? "进行中" :
                "已完成" }}</span>
            </template>
          </el-table-column>
    ...
    <div class="action_list">
      ...
      <div class="right">
        <!-- <el-button
          ...
          >一键确认提货</el-button
        > -->
        <el-button v-if="currentStep == 4" type="primary" @click="createDeliveryOrder"
          class="primary_btn">创建配送派工单</el-button>
        <!-- <el-button
          v-if="currentStep == 4"
          type="primary"
          ...
          >一键跳过配送</el-button
        > -->
        <el-button v-if="currentStep == 5" type="primary" @click="createInstallationOrder"
          class="primary_btn">创建安装派工单</el-button>
        ...
        <div class="content">
          ...
          <el-form v-if="currentDeliveryOrderStep == 2" :model="deliveryOrderForm" :rules="deliveryOrderRule"
            ...
            <el-form-item label="派工类型" prop="type">
              <el-input disabled placeholder="配送派工" v-model="deliveryOrderForm.type" />
            </el-form-item>
            ...
            <el-form-item label="上传图片" class="custom_upload">
              <el-upload :on-success="handleSuccessDelivery" :on-remove="handleDeleteDelivery" :auto-upload="true"
                ...
                v-model:file-list="deliveryOrderForm.fileList" class="avatar-uploader" :action=apiuploadUrl
    ...
    <div class="installationOrderDialog">
      <el-dialog v-model="installationOrderDialog" title="新建安装派工单" width="80%" :show-close="false">
        <div class="step">
          <span class="item">
            <span @click="changeInstallationOrderStep(1)" :class="currentInstallationOrderStep == 1 ? 'num_selected' : 'num'
              ">1</span>
          ...
          <span class="item">
            <span @click="changeInstallationOrderStep(2)" :class="currentInstallationOrderStep == 2 ? 'num_selected' : 'num'
              ">2</span>
          ...
          <span class="item">
            <span @click="changeInstallationOrderStep(3)" :class="currentInstallationOrderStep == 3 ? 'num_selected' : 'num'
              ">3</span>
        ...
        <div class="content">
          <el-form v-if="currentInstallationOrderStep == 1" :model="installationOrderForm" :rules="installationOrderRule"
            ...
            <el-form-item label="人员名称" prop="username">
              ...
              /> -->
              <el-select v-model="installationOrderForm.username" @change="onCahngeUserSelectForInstallion"
                placeholder="查找或输入服务人员姓名">
                <el-option v-for="item in extralUserData" :key="item.name" :label="item.name" :value="item.id" />
              </el-select>
            ...
            <el-form-item label="联系方式" prop="phone">
              <el-input placeholder="输入联系方式" v-model="installationOrderForm.phone" />
            </el-form-item>
            <el-form-item label="预约开始" prop="appointmentStartTime">
              <el-date-picker v-model="installationOrderForm.appointmentStartTime" type="datetime" placeholder="日期/时间"
                value-format="YYYY-MM-DD HH:mm:ss" ::default-time="defaultStartTime" :disabled-date="disabledPastDate" />
            </el-form-item>
            <el-form-item label="预约结束" prop="appointmentEndTime">
              <el-date-picker v-model="installationOrderForm.appointmentEndTime" type="datetime" placeholder="日期/时间"
                value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultStartTime" :disabled-date="disabledPastDate" />
            </el-form-item>
            <el-form-item label="安装小组">
              <span style="width: 300px">
                <el-checkbox v-model="installationOrderForm.installationTeam"></el-checkbox>
              </span>
            </el-form-item>
            <el-form-item label="添加组员" prop="installationTeam" v-if="installationOrderForm.installationTeam">
              <el-select multiple collapse-tags v-model="installationOrderForm.memberList" placeholder="请选择组员">
                <el-option v-for="item in extralUserData" :key="item.name" :label="item.name" :value="item.phone" />
              </el-select>
            </el-form-item>
          </el-form>
          <el-form v-if="currentInstallationOrderStep == 2" :model="installationOrderForm" :rules="installationOrderRule"
            ...
            <el-form-item label="派工单备注" class="customLayout">
              <el-input v-model="installationOrderForm.remark" :rows="5" type="textarea" maxlength="500"
                placeholder="请填写派工单备注" show-word-limit />
            </el-form-item>
            <el-form-item label="上传图片" class="custom_upload">
              <el-upload :on-success="handleSuccessInstall" :on-remove="handleDeleteInstall" :auto-upload="true"
                :data="uploadDataInstall" :headers="headers" :before-upload="beforeUploadInstall"
                :file-list="installationOrderForm.fileList" list-type="picture-card" class="avatar-uploader"
        ...
        <template #footer>
          <div class="dialog-footer">
            <el-button class="cancel_btn" @click="installationOrderDialog = false">取消</el-button>
            <el-button v-if="currentInstallationOrderStep == 1" type="primary" class="primary_btn"
              @click="installationOrderNextStep">下一步</el-button>
            <el-button v-if="currentInstallationOrderStep == 2" type="primary" class="primary_btn"
              @click="finishInstallationOrder">完成</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
    ...
    <div class="deliveryOrderDialog">
      <el-dialog v-model="editdeliveryOrderDialog" title="编辑配送派工单" width="80%" :show-close="false">
        <div class="step">
          <span class="item">
            <span @click="changeDeliveryOrderStep(1)"
              :class="currentDeliveryOrderStep == 1 ? 'num_selected' : 'num'">1</span>
        ...
        <div class="content">
          <el-form v-if="currentDeliveryOrderStep == 1" :model="editdeliveryOrderForm" :rules="deliveryOrderRule"
            ref="deliveryOrderFormRef" label-width="90px" label-position="left">
            <el-form-item label="人员名称" prop="followerId">
              <el-select v-model="editdeliveryOrderForm.followerId" @change="onCahngeUserSelectForDelivery"
                placeholder="查找或输入服务人员姓名">
                <el-option v-for="item in extralUserData" :key="item.name" :label="item.name" :value="item.id" />
              </el-select>
              <span class="custom_item" @click="changeToMyPerson"><img src="@/assets/images/add.png" alt="" /></span>
            ...
            <el-form-item label="联系方式" prop="contactTelephone">
              <el-input v-model="editdeliveryOrderForm.followerPhone" placeholder="查找或输入配送司机手机号码" />
            </el-form-item>

            <el-form-item label="预约开始" prop="appointmentStartTime">
              <el-date-picker v-model="editdeliveryOrderForm.appointmentStartTime" type="datetime" placeholder="日期/时间"
                value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultStartTime"
                :disabled-date="editdisabledPastDate" />
            </el-form-item>
            <el-form-item label="预约结束" prop="appointmentEndTime">
              <el-date-picker v-model="editdeliveryOrderForm.appointmentEndTime" type="datetime" placeholder="日期/时间"
                value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultStartTime"
                :disabled-date="editdisabledPastDate" />
            </el-form-item>
          </el-form>
        ...
        <template #footer>
          <div class="dialog-footer">
            <el-button class="cancel_btn" @click="editdeliveryOrderDialog = false">取消</el-button>
            <el-button v-if="currentDeliveryOrderStep == 1" type="primary" class="primary_btn"
              @click="editdeliveryOrderNextStep">提交</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
    <div class="installationOrderDialog">
      <el-dialog v-model="editinstallationOrderDialog" title="编辑安装派工单" width="80%" :show-close="false">
        <div class="step">
          <span class="item">
            <span @click="changeInstallationOrderStep(1)" :class="currentInstallationOrderStep == 1 ? 'num_selected' : 'num'
              ">1</span>
        ...
        <div class="content">
          <el-form v-if="currentInstallationOrderStep == 1" :model="editdeliveryOrderForm" :rules="installationOrderRule"
            label-width="80px" label-position="left">
            <el-form-item label="人员名称" prop="followerId">
              <el-select v-model="editdeliveryOrderForm.followerId" @change="onCahngeUserSelectForDelivery"
                placeholder="查找或输入服务人员姓名">
            ...
            <el-form-item label="联系方式" prop="followerPhone">
              <el-input placeholder="输入联系方式" v-model="editdeliveryOrderForm.followerPhone" />
            </el-form-item>
            <el-form-item label="预约开始" prop="appointmentStartTime">
              <el-date-picker v-model="editdeliveryOrderForm.appointmentStartTime" type="datetime" placeholder="日期/时间"
                value-format="YYYY-MM-DD HH:mm:ss" ::default-time="defaultStartTime"
        ...
        <template #footer>
          <div class="dialog-footer">
            <el-button class="cancel_btn" @click="editinstallationOrderDialog = false">取消</el-button>
            <el-button v-if="currentInstallationOrderStep == 1" type="primary" class="primary_btn"
              @click="editdeliveryOrderNextStep">提交</el-button>
            <!-- <el-button
        ...
        <div class="content">
          <el-form :model="problemReportingForm" :rules="problemReportingRule" label-width="90px" label-position="left">
            <el-form-item label="订单编号" prop="orderNo">
              <el-input placeholder="请选择订单编号" v-model="problemReportingForm.orderNo" />
            </el-form-item>
            <el-form-item label="客户姓名" prop="customerName">
              <el-select v-model="problemReportingForm.customerName" @change="onCahngeUserSelectForProblemReport"
                placeholder="查找或选择服务人员姓名">
                <el-option v-for="item in extralUserData" :key="item.name" :label="item.name" :value="item.id" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="上传图片" class="custom_upload">
              <el-upload :on-success="handleSuccessProblemReport" :on-remove="handleDeleteProblemReport"
                :auto-upload="true" :data="uploadDataProblemReport" :headers="headers"
                :before-upload="beforeUploadProblemReport" :file-list="problemReportingForm.fileList"
                list-type="picture-card" class="avatar-uploader" :action=apiuploadUrl :show-file-list="true">
                <el-icon class="avatar-uploader-icon">
                  <Plus />
                </el-icon>
              </el-upload>
    ...
    <!-- 实际包装数量修改 -->
    <div class="relatedDocumentsDialog">
      <el-dialog v-model="actualPkgCntDialogs" title="修改实际包装数量" width="500">
...
import { addFieldJob, getTaskByPage, getExternalUser, getFieldJob, updateDispatchNote, getOrderListById, getFeildJobList, getDispatchNoteByGet, getLoginInfo, createServiceCase, updateTask, getOrderList, getDispatchNoteList } from "../../api/common";
...
const getList = (orderId) => {
  ...
  getLoginInfo(param).then((res: any) => {
    ...
    if (dataInfo.length > 0) {
      ...
      taskStatus.value = route.query.status != null ? parseInt(route.query.status.toString(), 0) + 1 : 1
      taskDetails.value = {
